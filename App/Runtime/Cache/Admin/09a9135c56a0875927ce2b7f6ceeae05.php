...
<body>
    ...
    <ol class="breadcrumb">
        ...
        <li>
            <a href="/school/index.php/Admin/school/branch">分校科目管理</a>
        </li>
    ...
    <div class="layui-row layui-form">
        <div class="layui-col-md12">
            ...
            <div class="layui-btn-group">
                <button class="layui-btn layui-btn-sm layui-black " data-tips='3' data-color='#666' title='新增分校科目' id="add">
            ...
            layer.prompt({
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        ...
                        if (j != i) {
                            if (item_i['exam_title'] == item_j['exam_title']) {
                                //检测到重名
                                layer.msg('分校科目名：【' + item_i['exam_title'] + '】检测到冲突，请修改后再操作！');
                                layer.close(load);
                                return;
                            }
                        }
                ...
                (function () {
                    ...
                    var fun = function (res) {
                        ...
                        if (res.res > 0) {
                            //服务器有数据，进行判断
                            var exam_arr = res.msg;

                            for (var i = 0; i < arr.length; i++) {
                                var item_i = arr[i];

                                for (var j = 0; j < exam_arr.length; j++) {
                                    var item_j = exam_arr[j];

                                    if (item_i['exam_title'] == item_j['exam_title']) {
                                        //检测到重名
                                        w(item_i['exam_title']);
                                        w(item_j['exam_title']);
                                        layer.msg('分校科目名：【' + item_i['exam_title'] + '】已存在，请修改后再操作！');
                                        layer.close(load);
                                        return;
                                    }
                                }
