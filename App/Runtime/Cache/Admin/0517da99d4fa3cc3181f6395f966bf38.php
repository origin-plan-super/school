...
<head>
    ...
    <title>【<?php echo ($pages["info_pages_title"]); ?>】的图文详情管理</title>
    <!-- css -->
...
<body>
    ...
    <div class="page-header">
        <h1>
            <small>【<?php echo ($pages["info_pages_title"]); ?>】的图文详情管理</small>
        </h1>
    </div>
    ...
    <ol class="breadcrumb">
        ...
        <li class="active">【<?php echo ($pages["info_pages_title"]); ?>】的图文详情管理</li>
    </ol>
    ...
    <div class="layui-row layui-form">
        <div class="layui-col-md12">
            ...
            <div class="layui-btn-group">
                <button class="layui-btn layui-btn-sm layui-black " data-tips='3' data-color='#666' title='新增图文详情' id="add">
    ...
    <script type="text/html" id="bar1">
        ...
        <a class="layui-btn layui-btn-xs layui-black" lay-event="carousel" data-tips='4' title='管理图文详情'>
            <i class="layui-icon">&#xe614;</i>
        </a>
    ...
    <script type="text/html" id="checkboxTpl">
        <input type="checkbox" name="lock" value="{{d.info_pages_id}}" title="推荐" lay-filter="is_up" {{ d.is_up == 1 ? 'checked' : '' }}>
      </script>
    ...
    <script>
        ...
        layui.use(['table', 'form'], function () {
            ...
            tableIns = table.render({
                ...
                , cols: [[ //表头s
                    ...
                    , { field: 'info_pages_id', title: 'id', width: 100 }
                    , { field: 'info_pages_title', title: '图文详情', width: 200, edit: 'text' }
            ...
            table.on('tool(table_filter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'carousel') {
                    //管理轮播图
                    window.open('/school/index.php/Admin/carousel/showList/info_pages_id/' + data.info_pages_id);
                }
        ...
        */
        $(document).on('click', '#reload', function () {
            ...
            var key = $('#key').val();
            var key_where = 'info_pages_id|info_pages_title';
            //执行重载
            tableIns.reload({
                page: {
                    curr: 1 //重新从第 1 页s开始
            ...
            layer.prompt({
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        ...
                        if (j != i) {
                            if (item_i['info_pages_title'] == item_j['info_pages_title']) {
                                //检测到重名
                                layer.msg('图文详情名：【' + item_i['info_pages_title'] + '】检测到冲突，请修改后再操作！');
                                layer.close(load);
                                return;
                            }
                        }
                ...
                (function () {
                    ...
                    var fun = function (res) {
                        ...
                        if (res.res > 0) {
                            //服务器有数据，进行判断
                            var info_pages_arr = res.msg;

                            for (var i = 0; i < arr.length; i++) {
                                var item_i = arr[i];

                                for (var j = 0; j < info_pages_arr.length; j++) {
                                    var item_j = info_pages_arr[j];

                                    if (item_i['info_pages_title'] == item_j['info_pages_title']) {
                                        //检测到重名
                                        w(item_i['info_pages_title']);
                                        w(item_j['info_pages_title']);
                                        layer.msg('图文详情名：【' + item_i['info_pages_title'] + '】已存在，请修改后再操作！');
                                        layer.close(load);
                                        return;
                                    }
                                }
