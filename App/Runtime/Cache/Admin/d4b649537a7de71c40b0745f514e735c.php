...
<body>
    ...
    <div class="layui-row layui-form">
        <div class="layui-col-md12">
            ...
            <div class="layui-btn-group">
                <button class="layui-btn layui-btn-sm layui-black " data-tips='3' data-color='#666' title='新增电子书' id="add">
            ...
            <div class="layui-btn-group">
                ...
                <a class="layui-btn layui-btn-sm layui-black" data-tips='3' href='<?php echo U("Home/book/showList");?>' target="_blank" data-color='#666'
                    title='电子书列表'>
                    电子书列表
                </a>
    ...
    <script type="text/html" id="bar1">
        ...
        <a class="layui-btn layui-btn-xs layui-black" lay-event="edit" data-tips='4' title='去写作'>
            <!-- <i class="layui-icon">&#xe639;</i> -->
    ...
    <script type="text/html" id="checkboxTpl">
        <input type="checkbox" name="lock" value="{{d.book_id}}" title="推荐" lay-filter="is_up" {{ d.is_up == 1 ? 'checked' : '' }}>
    </script>
    ...
    <script>
        ...
        layui.use(['table', 'form'], function () {
            ...
            tableIns = table.render({
                ...
                , cols: [[ //表头s
                    ...
                    , { field: 'book_title', title: '标题', width: 200, edit: 'text' }
            ...
            table.on('tool(table_filter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'edit') {
                    //管理轮播图
                    window.open('/school/index.php/Admin/Book/edit/book_id/' + data.book_id);
                }
        ...
        */
        $(document).on('click', '#reload', function () {
            ...
            var key = $('#key').val();
            var key_where = 'book_id|book_title';
            //执行重载
            tableIns.reload({
                page: {
                    curr: 1 //重新从第 1 页s开始
                ...
                */
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        ...
                        if (j != i) {
                            if (item_i['book_title'] == item_j['book_title']) {
                                //检测到重名
                                layer.msg('电子书名：【' + item_i['book_title'] + '】检测到冲突，请修改后再操作！');
                                layer.close(load);
                                return;
                            }
                        }
                ...
                (function () {
                    ...
                    var fun = function (res) {
                        ...
                        if (res.res > 0) {
                            //服务器有数据，进行判断
                            var book_arr = res.msg;

                            for (var i = 0; i < arr.length; i++) {
                                var item_i = arr[i];

                                for (var j = 0; j < book_arr.length; j++) {
                                    var item_j = book_arr[j];

                                    if (item_i['book_title'] == item_j['book_title']) {
                                        //检测到重名
                                        w(item_i['book_title']);
                                        w(item_j['book_title']);
                                        layer.msg('电子书名：【' + item_i['book_title'] + '】已存在，请修改后再操作！');
                                        layer.close(load);
                                        return;
                                    }
                                }
