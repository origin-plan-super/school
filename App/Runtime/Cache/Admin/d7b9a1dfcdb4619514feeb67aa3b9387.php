...
<body>
    ...
    <script type="text/html" id="bar1">
        ...
        <a class="layui-btn layui-btn-xs layui-black" lay-event="carousel" data-tips='4' title='管理报名'>
            <i class="layui-icon">&#xe614;</i>
        </a>
    ...
    <script>
        ...
        layui.use(['table', 'form'], function () {
            ...
            tableIns = table.render({
                ...
                , even: true //开启隔行背景
                , elem: '#table'
                , url: '/school/index.php/Admin/Order/getList' //数据接口
                , where: {
                    table: 'order'
                }
                , response: {
                ...
                , cols: [[ //表头
                    ...
                    , { field: 'user_age', title: '年龄', width: 200 }
            ...
            table.on('tool(table_filter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'carousel') {
                    //管理轮播图
                    window.open('/school/index.php/Admin/carousel/showList/order_id/' + data.order_id);
                }
        ...
        */
        $(document).on('click', '#reload', function () {
            ...
            var key = $('#key').val();
            var key_where = 'order_id|subject_id|user_name|user_sex|user_mail|user_phone';
            //执行重载
            tableIns.reload({
                page: {
                    curr: 1 //重新从第 1 页s开始
                        ...
                        if (res.res > 0) {
                            ...
                            for (var i = 0; i < arr.length; i++) {
                                ...
                                for (var j = 0; j < order_arr.length; j++) {
                                    ...
                                    if (item_i['order_title'] == item_j['order_title']) {
                                        //检测到重名
                                        w(item_i['order_title']);
                                        w(item_j['order_title']);
                                        layer.msg('报名名：【' + item_i['order_title'] + '】已存在，请修改后再操作！');
                                        layer.close(load);
                                        return;
                                    }
                                }
