...
<body>
    ...
    <div class="layui-row layui-form">
        <div class="layui-col-md12">
            ...
            <div class="layui-btn-group">
                <a class="layui-btn layui-btn-sm layui-black" data-tips='3' href='/school/index.php/Admin/Paper/add' target="_blank" data-color='#666' title='新增页面'>
            ...
            <div class="layui-btn-group">
                <a class="layui-btn layui-btn-sm layui-black" data-tips='3' href='<?php echo U("Home/paper/panel");?>' target="_blank" data-color='#666'
                    title='看页面'>
                    虹桥面塑
                </a>
                <a class="layui-btn layui-btn-sm layui-black" data-tips='3' href='<?php echo U("Home/paper/tradition");?>' target="_blank" data-color='#666'
                    title='看页面'>
                    传统礼仪
                </a>
                <a class="layui-btn layui-btn-sm layui-black" data-tips='3' href='<?php echo U("Home/paper/originality");?>' target="_blank" data-color='#666'
                    title='看页面'>
                    独具匠心
                </a>
    ...
    <script type="text/html" id="checkboxTpl">
        <input type="checkbox" name="lock" value="{{d.paper_id}}" title="推荐" lay-filter="is_up" {{ d.is_up == 1 ? 'checked' : '' }}>
      </script>
    ...
    <script>
        ...
        layui.use(['table', 'form'], function () {
            ...
            tableIns = table.render({
                ...
                , cols: [[ //表头s
                    ...
                    , { field: 'paper_title', title: '文章标题', width: 400, edit: 'text' }
                    , { field: 'pages_title', title: '页面', width: 150, sedit: 'text' }
                    , { field: 'class_title', title: '分类', width: 150, sedit: 'text' }
            ...
            table.on('tool(table_filter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'edit') {
                    //管理轮播图
                    window.open('/school/index.php/Admin/Paper/edit/paper_id/' + data.paper_id);
                }
        ...
        */
        $(document).on('click', '#reload', function () {
            ...
            var key_where = 'paper_id|paper_title|pages_title|class_title';
            //执行重载
            tableIns.reload({
                page: {
                    curr: 1 //重新从第 1 页s开始
            ...
            layer.prompt({
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        ...
                        if (j != i) {
                            if (item_i['paper_title'] == item_j['paper_title']) {
                                //检测到重名
                                layer.msg('页面名：【' + item_i['paper_title'] + '】检测到冲突，请修改后再操作！');
                                layer.close(load);
                                return;
                            }
                        }
                ...
                (function () {
                    ...
                    var fun = function (res) {
                        ...
                        if (res.res > 0) {
                            //服务器有数据，进行判断
                            var paper_arr = res.msg;

                            for (var i = 0; i < arr.length; i++) {
                                var item_i = arr[i];

                                for (var j = 0; j < paper_arr.length; j++) {
                                    var item_j = paper_arr[j];

                                    if (item_i['paper_title'] == item_j['paper_title']) {
                                        //检测到重名
                                        w(item_i['paper_title']);
                                        w(item_j['paper_title']);
                                        layer.msg('页面名：【' + item_i['paper_title'] + '】已存在，请修改后再操作！');
                                        layer.close(load);
                                        return;
                                    }
                                }
